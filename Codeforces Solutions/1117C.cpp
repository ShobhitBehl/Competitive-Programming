...
void solve()
{
	...
	for(int i = 0; i<n; i++)
	{
		...
		c[i] = make_pair(xdiff,ydiff);
	}
	lli left = 0, right = 1e18,ans = 1e18;
	while(left <= right)
	{
		lli mid = (left+right)/2;
		lli num = mid/n;
		lli rem = mid%n;
		lli totalxdiff = c[n-1].first*num;
		lli totalydiff = c[n-1].second*num;
		if(rem != 0)
		{
			totalxdiff += c[rem-1].first;
			totalydiff += c[rem-1].second;
		}
		//cout << totalxdiff << " " << totalydiff << endl;
		lli tempx = x1 + totalxdiff;
		lli tempy = y1 + totalydiff;
