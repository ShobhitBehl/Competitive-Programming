...
int main()
{
    ...
    while(iter < 100)
    {
        ...
        long double R = (l+r)/2.00000000000000;
        lli flag = 1;
        for(int i = 0; i<n; i++)
        {
            if((2*R*a[i].y) - (a[i].y*a[i].y) >= 0)
            {
                long double temp = sqrt((2*R*a[i].y) - (a[i].y*a[i].y));
                //tr(temp,a[i].x,R);
                maxx = max(maxx,a[i].x - temp);
                minx = min(minx,a[i].x + temp);
            }
            else
            {
                flag = 0;
