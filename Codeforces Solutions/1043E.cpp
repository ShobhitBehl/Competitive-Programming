...
void solve()
{
    ...
    lli fsum[n+1],ssum[n+1];
    fsum[0] = 0;
    ssum[0] = 0;
    //cout << endl;
    for(int i = 1; i<=n; i++)
    {
        fsum[i] = fsum[i-1] + a[i].f;
        ssum[i] = ssum[i-1] + a[i].s;
        //cout << fsum[i] << " " << ssum[i] << endl;
    }
    for(int i = 1; i<=n; i++)
    {
        //cout << a[i].f << " " << a[i].s << endl;
        ans[ind[a[i]]] = (a[i].f)*(n-i) + a[i].s*(i-1) + ssum[n] - ssum[i] + fsum[i-1];
    }
    for(int i = 0; i<m; i++)
    {
        lli u,v;
        cin >> u >> v;
        ans[u] -= min(b[u].f + b[v].s, b[u].s + b[v].f);
        ans[v] -= min(b[u].f + b[v].s, b[u].s + b[v].f);
    }
    for(int i = 1; i<=n; i++)
    {
        cout << ans[i] << " ";
