...
void solve()
{
    ...
    {
        cin >> a[i];
        b[a[i]%3].insert(mp(a[i],i+1));
    }
    lli curr = 0;
    for(int i = 0; i<n; i++)
    {
        set <pll> :: iterator it = b[curr%3].lower_bound(mp(curr,-1));
        if(it != b[curr%3].end() && it->first == curr)
        {
            ans.pb(it->second);
        }
        else
        {
            if(it != b[curr%3].begin())
            {
                it--;
                ans.pb(it->second);
            }
