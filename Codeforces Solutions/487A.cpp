...
int main()
{
    ...
    cin.tie(NULL);
    lli hm,am,dm,hy,ay,dy,h,a,d;
    cin >> hy >> ay >> dy >> hm >> am >> dm >> h >> a >> d;
    int ta,td, mina = INT_MAX, mind = INT_MAX, mincost = INT_MAX;
    for(int i = ay; i<=200; i++)
    {
        for(int j = dy; j<=100; j++)
        {
            ...
            if(ta > dm)
            {
                ...
                {
                    neededhp = time*(am-dy) - hy + 1;
                }
                //cout << ta << " " << td << " " << time << " " << neededhp << endl;
                if(neededhp*h + (ta-ay)*a + (td-dy)*d < mincost)
                {
                    mincost = neededhp*h + (ta-ay)*a + (td-dy)*d;
                }
            }
        }
    }
